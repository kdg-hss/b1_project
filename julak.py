...
(VMESS_GET_USERNAME, VMESS_GET_EXPIRED_DAYS, TROJAN_GET_USERNAME, TROJAN_GET_EXPIRED_DAYS,
 ...
 NOMINAL, KONFIRMASI, SELESAI,
 GET_VMESS_USER_TO_DELETE, GET_SHADOWSOCKS_USER_TO_DELETE) = range(35)
...
async def cancel_topup_auto(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if kode_deposit:
        try:
            requests.post(
                "https://ariepulsa.my.id/api/qrisrealtime",
                data={
    ...
    try:
        ...
        if not image_sent:
            ...
            caption_text = (
                ...
                f"🧾 Kode Deposit: `{kode_deposit}`\n⏰ Berlaku sampai: {expired}\n\n"
                f"Setelah bayar, tekan tombol *✅ Konfirmasi Pembayaran*."
            )
